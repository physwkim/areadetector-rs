...
fn main() {
    ...
    handle.write_int32_blocking(rt.ad_params.base.data_type, 0, 3).unwrap(); // UInt16
    handle.write_int32_blocking(rt.ad_params.image_mode, 0, ImageMode::Continuous as i32).unwrap();
    handle.write_float64_blocking(rt.ad_params.acquire_time, 0, 0.01).unwrap();
    handle.write_float64_blocking(rt.ad_params.acquire_period, 0, 0.05).unwrap();
    ...
    for i in 0..10 {
        ...
        let size_x = handle.read_int32_blocking(rt.ad_params.base.array_size_x, 0).unwrap_or(0);
        let size_y = handle.read_int32_blocking(rt.ad_params.base.array_size_y, 0).unwrap_or(0);
        let total = handle.read_int32_blocking(rt.ad_params.base.array_size, 0).unwrap_or(0);
        println!(
    ...
    let final_count = handle.read_int32_blocking(rt.ad_params.base.array_counter, 0).unwrap();
