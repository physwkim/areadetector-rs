...
fn test_mode_switch_during_continuous() {
    ...
    handle.write_int32_blocking(rt.sim_params.sim_mode, 0, 0).unwrap();
    ...
    handle.write_int32_blocking(rt.sim_params.sim_mode, 0, 1).unwrap();
...
fn test_rgb1_mode_acquisition() {
    ...
    handle.write_int32_blocking(rt.ad_params.image_mode, 0, ImageMode::Single as i32).unwrap();
    handle.write_int32_blocking(rt.ad_params.base.color_mode, 0, ColorMode::RGB1 as i32).unwrap();
    handle.write_float64_blocking(rt.ad_params.acquire_time, 0, 0.001).unwrap();
    handle.write_float64_blocking(rt.ad_params.acquire_period, 0, 0.001).unwrap();
